...
%% load and scale NGRIP data
% North Greenland Ice Core Project members (2004)
% High-resolution record of Northern Hemisphere climate extending into the
% last interglacial period. Nature 431(7005):147-151. 
data=csvread('NGRIP.csv');
NGRIP.age=data(:,1)';
NGRIP.d8Otemp=data(:,2)';
p = polyfit(NGRIP.d8Otemp,interp1(LR04.age,LR04.d18O,NGRIP.age),1);
NGRIP.d18O=polyval(p,NGRIP.d8Otemp); % scale to LR04
...
%% combine curves
climatecurves.age=[NGRIP.age,LR04.age(LR04.age>max(NGRIP.age)),ZACHOS.age(ZACHOS.age>max(LR04.age))];
climatecurves.d18O=[NGRIP.d18O,LR04.d18O(LR04.age>max(NGRIP.age)),ZACHOS.d18O(ZACHOS.age>max(LR04.age))];
...
plot(NGRIP.age,NGRIP.d18O,'-r','LineWidth',3)
plot(climatecurves.age,climatecurves.d18O,'-','Color','k','LineWidth',1)
legend('Zachos+Hansen','LR04','NGRIP',climatecurves.ver,...
    'Location','southwest')
ylabel('\delta^{18}O')
...
%plot(NGRIP.age,NGRIP.d18O,'-r','LineWidth',2)
%plot(climatecurves.age,climatecurves.d18O,'-','Color','k','LineWidth',1)
%legend('Zachos+Hansen','LR04','NGRIP',climatecurves.ver,...
%    'Location','southeast')
%ylabel('\delta^{18}O')
